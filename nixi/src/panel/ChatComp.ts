class ChatComp extends eui.Component {
		...
		if (this.editText.text == "") {
			...
			if (IllegalWords.is_learnWords(this.editText.text)) {
				...
				if (ChatData.isChatOk) {
					...
					// Prompt.showPrompt(this.stage, "发言频率过快," + (this.timer.repeatCount - this.timer.currentCount) + "秒后可继续发言");
	...
	private updateSendTime() {
		...
		// ChatData.lastChatTime = now_time;
		...
		// }else {
		// 	ChatData.chatCount = newChatCount;
		// }
	}
